...
We are going to cluster it using a density based approach with the use of the \texttt{DBSCAN}. The settings used (\textit{epsilon} and \textit{minPts}) have been tuned manually to get the best results.
...
As we can see in figures \ref{DBSCAN AccelerometerDay1}, \ref{DBSCAN AccelerometerDay2} and \ref{DBSCAN Gyroscope} 
DBSCAN is not able to correctly cluster the data because it only groups the data in 2 clusters and some data points are lost because they are not assigned any cluster.
 
Because of this we are not going to use \texttt{DBSCAN} based clustering in our interpretations.
